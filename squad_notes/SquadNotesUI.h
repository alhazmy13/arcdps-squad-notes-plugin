...
class SquadNotesUI : public MainWindow, public Singleton<SquadNotesUI> {
public:
	SquadNotesUI();
...
private:
	...
	std::unique_ptr<SquadNotesUITable> mTable;

	std::deque<std::string> mDataToPrint;
	std::mutex mDataToPrintMutex;

	std::string mTitleDefault = "Squad Notes";
	std::string mWindowId = "Squad Notes";
	std::string mAppearAsInOptionDefault = "Squad Notes";
};
