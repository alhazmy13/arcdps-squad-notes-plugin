...
uintptr_t mod_windows(const char* windowname) {
	if (!windowname) {
		SquadNotesUI::instance().DrawOptionCheckbox();
	}
	return 0;
}
...
uintptr_t mod_imgui(uint32_t not_charsel_or_loading) {
	...
	UpdateChecker::instance().Draw(GlobalObjects::UPDATE_STATE, SQUAD_NOTES_PLUGIN_NAME, "https://github.com/alhazmy13/arcdps-squad-notes-plugin/releases/latest");

	if (not_charsel_or_loading) {
		SquadNotesUI::instance().Draw(!GlobalObjects::CanMoveWindows() ? ImGuiWindowFlags_NoMove : 0);

		// load squad notes if window was just opened
		bool& showSquadNote = Settings::instance().settings.showSquadNotes;

		lastFrameShow = showSquadNote;
	}
			...
			if (!src->elite) {
				...
				if (src->name != nullptr && src->name[0] != '\0' && dst->name != nullptr && dst->name[0] != '\0') {
					...
					if (src->prof) {
						...
						SquadNotesUI::instance().GetTable()->RequestSort();
					}
					/* remove */
					else {
				...
				if (src->prof) {
					...
					if (dst->self) {
						...
						if (mapViewOfMumbleFile) {
							...
							SquadNotesUI::instance().GetTable()->ResetSpecificColumnSetup();
						}
					}
				}
			}
...
arcdps_exports* mod_init() {
		...
		if (currentVersion) {
			GlobalObjects::UPDATE_STATE = std::move(
				updateChecker.CheckForUpdate(SELF_DLL, currentVersion.value(),
															  "alhazmy13/arcdps-squad-notes-plugin", false));
		}
	...
	arc_exports.out_name = SQUAD_NOTES_PLUGIN_NAME;
...
extern "C" __declspec(dllexport) void arcdps_unofficial_extras_subscriber_init(
	...
	if (pExtrasInfo->ApiVersion == 1) {
		...
		extrasSubscriberInfo->SubscriberName = SQUAD_NOTES_PLUGIN_NAME;
	...
	// MaxInfoVersion has to be higher to have enough space to hold this object
		...
		subscriberInfo->SubscriberName = SQUAD_NOTES_PLUGIN_NAME;
